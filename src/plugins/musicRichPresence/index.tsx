...
export default definePlugin({
    ...
    settingsAboutComponent() {
        return (
            <Card>
                ...
                <Paragraph>
                    Set <strong>Application name</strong> and <strong>Application description</strong> to anything and leave the rest blank.
                </Paragraph>
                <LinkButton size="small" href="https://www.last.fm/api/account/create" className={Margins.top8}>
                    Create API Key
                </LinkButton>
            </Card>
        )
    ...
    getLargeImage(track: TrackData): string | undefined {
        if (settings.store.showAlbumCover && track.imageURL && !track.imageURL.includes(LASTFM_PLACEHOLDER_IMAGE_HASH)) return track.imageURL
    ...
    async getActivity(): Promise<Activity | null> {
        ...
        if (settings.store.hideWithActivity) {
            if (
                PresenceStore.getActivities(AuthenticationStore.getId()).some(
                    (a) => a.application_id !== DISCORD_APP_ID && a.type !== ActivityType.CUSTOM_STATUS,
                )
            ) {
                return null
            }
        ...
        if (settings.store.hideWithSpotify) {
            if (
                PresenceStore.getActivities(AuthenticationStore.getId()).some(
                    (a) => a.type === ActivityType.LISTENING && a.application_id !== DISCORD_APP_ID,
                )
            ) {
