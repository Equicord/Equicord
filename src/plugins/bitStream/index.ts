...
const PAN_EDGE_SLACK = 0.25;

function clampPan(pan: number, size: number, zoom: number) {
    const max = (size * Math.max(zoom - 1, 0)) / 2 + size * PAN_EDGE_SLACK;
...
function findVideo(userId: string) {
    const tiles = Array.from(document.querySelectorAll<HTMLElement>("[data-selenium-video-tile]"));
    const videos = tiles
        .filter(t => normalizeId(t.getAttribute("data-selenium-video-tile") ?? "") === userId)
        .flatMap(t => Array.from(t.querySelectorAll<HTMLVideoElement>("video")));
...
function TileOverlay({ participantId }: { participantId: string; }) {
    ...
    const sel = `[data-selenium-video-tile="${CSS.escape(participantId)}"]`;
