...
const cl = classNameFactory("vc-frequentfriends-");
...
function UserAvatar({ userId, isFirst, isLast }: {
    ...
    const avatarDecorationUrl = useMemo(() => {
        const asset = user?.avatarDecorationData;
        if (!asset) return undefined;
        return (IconUtils as any).getAvatarDecorationURL({ avatarDecoration: asset, canAnimate: isHovered, size: 48 });
    ...
    if (isFirst) {
        badge = (
            <Tooltip text="Most Frequent" position="top">
                {(props: any) => (
                    <div {...props} className={`${cl("badge")} ${cl("badge-fire")}`}>
        ...
        badge = (
            <Tooltip text="Cooling off" position="top">
                {(props: any) => (
                    <div {...props} className={`${cl("badge")} ${cl("badge-snow")}`}>
    ...
    return (
        ...
        >
            <Tooltip text={name} position="top">
                {(props: any) => (
                    <div {...props} className={cl("avatar-inner-wrapper")}>
                        <Avatar
                            className={cl("avatar-inner")}
...
function InfoTooltip() {
    return (
        <Tooltip text="Friends you talk to the most in chat and voice channels" position="top">
            {(props: any) => (
                <div {...props} className={cl("info-wrapper")}>
                    <span className={cl("info-icon")}>i</span>
                </div>
            )}
        </Tooltip>
    );
    ...
    return (
        <div className={cl("root")}>
            <div className={cl("label")}>
                <span className={cl("label-text")}>{label}</span>
                <InfoTooltip />
            </div>
            {ids.length === 0 ? (
                <div className={cl("empty")}>No frequent friends yet.</div>
            ) : (
                <div className={cl("avatars")}>
                    {ids.map((id, i) => (
                        <UserAvatar
                            key={id}
                            userId={id}
