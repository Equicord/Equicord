...
export function PluginCard({ plugin, disabled, onRestartNeeded, onMouseEnter, onMouseLeave, isNew }: PluginCardProps) {
    ...
    const pluginInfo = [
        ...
        {
            condition: isOutcordPlugin,
            src: "https://i.imgur.com/6SKWKO0.png",
            alt: "Outcord",
            title: "Outcord Plugin",
        },
        {
    ...
    const sourceBadge = pluginDetails ? <img src={pluginDetails.src} alt={pluginDetails.alt} className={cl("source")} /> : null
    ...
    return (
        <AddonCard
            ...
            infoButton={
                <button role="switch" onClick={() => openPluginModal(plugin, onRestartNeeded)} className={cl("info-button")}>
                    {hasAnyVisibleSettings(plugin) ? <CogWheel className={cl("info-icon")} /> : <InfoIcon className={cl("info-icon")} />}
                </button>
            }
        />
    )
