...
export function SetAvatarModal({ userId, modalProps }: { userId: string, modalProps: ModalProps; }) {
    ...
    return (
        <ModalRoot {...modalProps}>
            <ModalHeader className={cl("modal-header")}>
                <Heading tag="h2">Custom Avatar</Heading>
                <ModalCloseButton onClick={modalProps.onClose} />
            </ModalHeader>
            ...
            <ModalFooter className={cl("modal-footer")}>
                <Button variant="dangerPrimary" onClick={deleteUserAvatar}>Delete Entry</Button>
                <Button variant="primary" onClick={saveUserAvatar}>Save</Button>
            </ModalFooter>
        </ModalRoot>
    );
}
