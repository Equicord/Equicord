...
type RecentAvatarsStoreLike = { getAvatars?: () => RecentAvatarEntry[] | undefined; };
...
export class ProfileRecentsRuntime {
    ...
    private refreshRecents(force = false) {
        ...
        if (force) FluxDispatcher.dispatch({ type: "RECENT_AVATARS_FETCH_SUCCESS", avatars: [...this.getNativeAvatars()] });
    }

    private getNativeAvatars(): RecentAvatarEntry[] {
        try {
            const store = RecentAvatarsStore as RecentAvatarsStoreLike | undefined;
    ...
    private async isDuplicateNative(dataUrl: string): Promise<boolean> {
        ...
        const natives = this.getNativeAvatars();
    ...
    private applyPendingPreview(dataUrl: string) {
        ...
        if (this.kind === "avatar") {
            const avatar = {
                ...
                imageUri: dataUrl,
                description: `profilerecents-avatar, added ${new Date().toLocaleString()}`
            };
            const type = guildId ? "GUILD_IDENTITY_SETTINGS_SET_PENDING_AVATAR" : "USER_PROFILE_SETTINGS_SET_PENDING_AVATAR";
            FluxDispatcher.dispatch(guildId ? { type, guildId, avatar } : { type, avatar });
            return;
        }

        const type = guildId ? "GUILD_IDENTITY_SETTINGS_SET_PENDING_BANNER" : "USER_PROFILE_SETTINGS_SET_PENDING_BANNER";
        FluxDispatcher.dispatch(guildId ? { type, guildId, banner: dataUrl } : { type, banner: dataUrl });
    }
