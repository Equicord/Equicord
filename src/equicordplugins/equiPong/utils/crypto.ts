...
const SHARE_LINK_DOMAIN = "youtube.com";
...
export function buildShareMessage(encrypted: string) {
    ...
    return `[${SHARE_LINK_LABEL}](https://${SHARE_LINK_DOMAIN}/${encoded})`;
}

export function parseShareMessage(content: string) {
    const match = content.match(/^\[Equipong\]\(https:\/\/youtube\.com\/([A-Za-z0-9._~%-]+)\)$/);
