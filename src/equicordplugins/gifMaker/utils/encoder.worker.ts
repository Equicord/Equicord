    ...
    try {
        switch (msg.type) {
            ...
            case "frame": {
                ...
                var index = applyPalette(pixels, pal);
                _gif.writeFrame(index, self.__w, self.__h, {
                    delay: self.__delay,
                    palette: idx === 0 ? pal : undefined,
                });
                self.postMessage({ type: "ok" });
                break;
            }
            case "finish": {
                var _gif2 = self.__gif;
                _gif2.finish();
                var bytes = _gif2.bytesView();
                var out = new Uint8Array(bytes.length);
                out.set(bytes);
                self.postMessage({ type: "result", buffer: out.buffer }, [out.buffer]);
                delete self.__gif;
                delete self.__palette;
                break;
            }
        }
    } catch (err) {
...
export function createGifEncoderWorker(): Worker {
    const blob = new Blob([GIFENC_SOURCE], { type: "text/javascript" });
    const url = URL.createObjectURL(blob);
    const worker = new Worker(url);
    Object.defineProperty(worker, "__blobUrl", { value: url });
    return worker;
}

export function terminateWorker(worker: Worker) {
    const url = (worker as any).__blobUrl as string | undefined;
