...
export function measureTextLines(ctx: CanvasRenderingContext2D, text: string, fontSize: number, maxWidth: number): MeasureResult {
...
export const captionCaption: CaptionDefinition = {
    ...
    render: (ctx, width, captionHeight, options) => {
        const { captionText, captionSize } = options;
        if (!captionText || captionHeight <= 0) return;

        const maxWidth = width - 20;
        const { lines, lineHeight } = measureTextLines(ctx, captionText, captionSize, maxWidth);
        const padding = 10;
