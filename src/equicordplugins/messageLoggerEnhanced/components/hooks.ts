...
function getSearchableMessageText(record: DBMessageRecord) {
    ...
    collectSearchText(record.message, parts);

    return parts.join("\n").toLowerCase();
}

function collectSearchText(value: unknown, parts: string[], depth = 0) {
    ...
    switch (typeof value) {
        ...
        case "object":
            if (Array.isArray(value)) {
                for (const item of value) collectSearchText(item, parts, depth + 1);
                return;
            }

            for (const child of Object.values(value as Record<string, unknown>)) {
                collectSearchText(child, parts, depth + 1);
            }
    }
}
