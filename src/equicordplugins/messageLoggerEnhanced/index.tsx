...
import { ChannelStore, FluxDispatcher, MessageActions, MessageStore, SelectedChannelStore, UserStore } from "@webpack/common";
...
export default definePlugin({
    ...
    async start() {
        ...
        if (settings.store.enableReplyToDeleted && MessageActions) {
            this.unpatchSendMessage = checkAndPatch(MessageActions, "sendMessage");
        }
    },

    unpatchSendMessage: null as (() => void) | null,

    stop() {
        removeContextMenuBindings();
        MessageStore.getMessage = this.oldGetMessage;
        if (this.unpatchSendMessage) {
            this.unpatchSendMessage();
            this.unpatchSendMessage = null;
        }
    }
});
