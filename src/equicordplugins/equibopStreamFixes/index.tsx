...
const settings = definePluginSettings({
	...
	keyframeInterval: {
		...
		description:
			"Keyframe interval in ms (0 = encoder default, 5000 = every 5s)",
		default: 5000,
		restartNeeded: true,
	},
	minBitrate: {
	...
	bitsPerPixelPct: {
		type: OptionType.NUMBER,
		description:
			"Bits per pixel percentage for target bitrate (8 = 0.08 bpp, 12 = 0.12bpp)",
		default: 8,
		restartNeeded: false,
	},
});
...
export default definePlugin({
	...
	getStreamConfig() {
		...
		const bitrateTarget = Math.min(
			20000000,
			Math.max(1500000, Math.round(pixelRate * bpp)),
		);
