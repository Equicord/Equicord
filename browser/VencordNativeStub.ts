...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/7xeh/Equicord" }),
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("XehcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("XehcordSettings", JSON.stringify(s)),
